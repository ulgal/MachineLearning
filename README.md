...
## Dimension  
차원 축소  
- 차원이 커지면 공간의 크기가 기하 급수적으로 커져 데이터의 밀도가 급격히 낮아지므로 데이터의 분포 및 모델 분석에 필요한 샘플의 수가 기하급수적으로 늘어난다. 따라서 일반적으로 차원의 크기를 줄이고 핵심이 되는 feature들을 선별하여 데이터의 분포 및 모델 분석을 한다. 주로 PCA 등을 사용.  
차원 확장  
